<x-sidebar-layout title="Reports & Analytics">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            console.log('Initializing with This Month period...');
            setDateFiltersForPeriod('month');
            
            // Ensure the period selector is set correctly
        ...
        function setDateFiltersForPeriod(period) {
            ...
            switch (period) {
                ...
                case 'month':
                    // This month - ensure we get the full month correctly
            ...
            // Set the date filter inputs with proper formatting
            const fromDateString = startDate.toISOString().split('T')[0];
            const toDateString = endDate.toISOString().split('T')[0];
            
            document.getElementById('dateFromFilter').value = fromDateString;
            document.getElementById('dateToFilter').value = toDateString;
            
            console.log('Date inputs set:', {
                fromInput: fromDateString,
                toInput: toDateString,
                fromElement: document.getElementById('dateFromFilter').value,
                toElement: document.getElementById('dateToFilter').value
            });
        }
        ...
        async function loadDashboardDataForCustomRange(dateFrom, dateTo) {
            try {
                // For Branch Pastor, always use their assigned branch
        ...
        async function loadDashboardData() {
            try {
                ...
                // Load dashboard statistics
                console.log('Making API call to:', `/api/reports/dashboard?${params}`);
                const response = await fetch(`/api/reports/dashboard?${params}`, {
                    headers: {
                        'Accept': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
        ...
        function updateDashboardStats(data) {
            ...
            if (data.attendance && typeof data.attendance.total_attendance !== 'undefined') {
                ...
                console.log('totalAttendanceElement found:', totalAttendanceElement);
                if (totalAttendanceElement) {
                    ...
                    console.error('totalAttendance element not found!');
                }
            } else {
                console.log('No attendance data found:', data);
            }
        ...
        async function loadEventReports(page = 1, perPage = null) {
            try {
                ...
                // Add branch filter - For Branch Pastor, always use their assigned branch
                const branchId = @json(Auth::user()->getActiveBranchId());
        ...
        function debounceReportEvents(fn, ms) {
            let t;
        ...
        function reportEventsBranchIdPastor() {
            return @json(Auth::user()->getActiveBranchId());
        }

        async function populateReportEventSelect(selectEl, branchId, search) {
            if (!selectEl) {
                return;
            }
            const params = new URLSearchParams({
        ...
        async function loadEvents() {
            try {
                const branchId = reportEventsBranchIdPastor();
                ...
                await Promise.all([
                    populateReportEventSelect(document.getElementById('eventSelect'), branchId, ''),
                    populateReportEventSelect(document.getElementById('editEventSelect'), branchId, ''),
                ]);
            } catch (error) {
        ...
        function setupReportEventSearchListeners() {
            ...
            const editSearch = document.getElementById('editEventSelectSearch');
            if (createSearch) {
                createSearch.addEventListener('input', debounceReportEvents(() => {
                    populateReportEventSelect(document.getElementById('eventSelect'), branchId(), createSearch.value);
                }, 300));
            }
            if (editSearch) {
                editSearch.addEventListener('input', debounceReportEvents(() => {
                    populateReportEventSelect(document.getElementById('editEventSelect'), branchId(), editSearch.value);
                }, 300));
            }
        }
        ...
        async function exportToPDF() {
            try {
                ...
                // Get branch information - For Branch Pastor, use their assigned branch
                const branchName = '{{ Auth::user()->getPrimaryBranch()?->name ?? "Your Branch" }}';
                ...
                // Add branch to filename for Branch Pastor
                ...
                if (dateFrom && dateTo) {
                    ...
                    switch(period) {
                        case 'week':
                            ...
                            break;
                        case 'month':
                            // This month - consistent with main function
                            const currentYear = now.getFullYear();
                            const currentMonth = now.getMonth();
                            startDate = new Date(currentYear, currentMonth, 1);
                            endDate = new Date(currentYear, currentMonth + 1, 0);
        ...
        function initializeGlobalReport() {
            ...
            // Event listeners
            const generateGlobalReportBtn = document.getElementById('generateGlobalReportBtn');
            if (generateGlobalReportBtn) {
                generateGlobalReportBtn.addEventListener('click', generateGlobalReport);
            }
            
            @if(auth()->user()->isSuperAdmin())
            const generateAllBranchesReportBtn = document.getElementById('generateAllBranchesReportBtn');
            if (generateAllBranchesReportBtn) {
                generateAllBranchesReportBtn.addEventListener('click', generateAllBranchesReport);
            }
            @endif
        }
        ...
        // Token Management Functionality for Branch Pastor
        let tokens = [];
        let availableEvents = [];
    ...
    <div id="tokenManagementModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden z-50" onclick="if(event.target === this) this.classList.add('hidden')">
    ...
    <!-- Create Token Modal -->
    <div id="createTokenModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden z-50" onclick="if(event.target === this) this.classList.add('hidden')">
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div class="mt-3">
    ...
    <!-- Edit Token Modal -->
    <div id="editTokenModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden z-50" onclick="if(event.target === this) this.classList.add('hidden')">
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div class="mt-3">
