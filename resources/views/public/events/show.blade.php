...
<head>
    ...
    <title>{{ config('app.name', 'LifePointe') }} - Events</title>
    ...
    <style>
        :root {
            --e-global-color-primary: #DD5D20;
            --e-global-color-secondary: #DD5D20;
    ...
    <div class="min-h-screen">
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between h-20">
                    ...
                    <div class="hidden md:flex items-center space-x-8">
                        <a href="{{ route('public.events') }}" class="text-[#DD5D20] hover:text-white px-3 py-2 rounded-md text-sm font-medium">
                            Events
                        </a>
                    ...
                    <div class="flex items-center space-x-4">
                        <!-- Login Button -->
                        <a href="{{ route('login') }}" class="bg-[#DD5D20] hover:bg-[#C24E16] text-white px-4 py-2 rounded-md text-sm font-medium transition-colors">Login</a>
                ...
                <div x-show="mobileMenuOpen" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95" class="md:hidden">
                    <div class="px-2 pt-2 pb-3 space-y-1 sm:px-3 bg-gray-800 rounded-lg mt-2">
                        <a href="{{ route('public.events') }}" class="text-[#DD5D20] hover:text-white block px-3 py-2 rounded-md text-base font-medium">
                            Events
                        </a>
        ...
        <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
            ...
            <nav class="mb-6 sm:mb-8 text-sm" aria-label="Breadcrumb">
                <a href="{{ route('public.events') }}" class="text-[#DD5D20] hover:underline font-medium">Events</a>
            ...
            <div class="flex flex-col lg:flex-row lg:items-start gap-8 sm:gap-10 lg:gap-10 xl:gap-12">
                ...
                <aside class="w-full lg:w-[34%] lg:max-w-none lg:flex-shrink-0 space-y-6 sm:space-y-7">
                    ...
                    <div class="rounded-2xl bg-white p-5 sm:p-6 border border-[#FFA589]/35 shadow-sm">
                        <h2 class="text-[11px] font-bold uppercase tracking-[0.2em] text-[#DD5D20] mb-4">Expression</h2>
                        @if($branch)
                            <div class="space-y-3">
                                @if($branch->logo_url)
                                ...
                                @if($branch->phone)
                                    <p class="text-sm">
                                        <a href="tel:{{ preg_replace('/\s+/', '', $branch->phone) }}" class="font-semibold text-[#DD5D20] hover:text-[#D44A1F] transition-colors">{{ $branch->phone }}</a>
                                    </p>
                                @endif
                                @if($branch->email)
                                    <p class="text-sm break-all">
                                        <a href="mailto:{{ $branch->email }}" class="text-[#DD5D20] font-medium hover:underline">{{ $branch->email }}</a>
                                    </p>
                                @endif
                            </div>
                        @else
                    ...
                    {{-- C. Event type --}}
                    <div class="rounded-2xl bg-white p-5 sm:p-6 border border-[#DD5D20]/30 shadow-sm">
                        <h2 class="text-[11px] font-bold uppercase tracking-[0.2em] text-[#DD5D20] mb-3">Event type</h2>
                        ...
                        <span class="inline-flex mt-4 px-2.5 py-1 rounded-full text-[11px] font-semibold uppercase tracking-wide bg-[#DD5D20]/18 text-[#1E1E1E] border border-[#DD5D20]/25">#{{ \Illuminate\Support\Str::slug($typeLabel) }}</span>
                    </div>
                </aside>
                        ...
                        <div class="flex flex-col gap-3 sm:gap-3.5 pt-1 border-t border-[#FFAC93]/25">
                            @if($start)
                                <div class="flex items-start gap-3">
                                    <span class="shrink-0 w-9 h-9 rounded-lg bg-[#DD5D20]/10 flex items-center justify-center text-[#DD5D20]" aria-hidden="true">
                            ...
                            @if($event->max_capacity)
                                <div class="flex items-start gap-3">
                                    <span class="shrink-0 w-9 h-9 rounded-lg bg-[#DD5D20]/15 flex items-center justify-center text-[#DD5D20]" aria-hidden="true">
                    ...
                    <div class="rounded-2xl bg-white p-6 sm:p-7 border border-[#FFA589]/45 shadow-sm">
                        ...
                        @elseif($event->registration_type === 'link' && $event->registration_link)
                            ...
                            <a href="{{ $event->registration_link }}" target="_blank" rel="noopener noreferrer"
                               class="inline-flex items-center justify-center w-full px-6 py-3.5 rounded-xl font-semibold text-white bg-[#DD5D20] hover:bg-[#D44A1F] focus:outline-none focus:ring-2 focus:ring-[#DD5D20] focus:ring-offset-2 transition-colors shadow-sm">
                                Register
                            </a>
                        @elseif(in_array($event->registration_type, ['simple', 'form'], true))
                            ...
                            <form id="publicEventRegisterForm" class="space-y-4">
                                <div>
                                    <label for="reg_name" class="block text-sm font-semibold text-[#1E1E1E] mb-1.5">Full name <span class="text-[#DD5D20]">*</span></label>
                                    <input type="text" id="reg_name" name="name" required
                                           class="w-full px-4 py-3 rounded-xl border border-[#FFAC93]/50 bg-white text-[#1E1E1E] placeholder:text-[#606060]/45 focus:outline-none focus:ring-2 focus:ring-[#DD5D20] focus:border-[#DD5D20]/50">
                                </div>
                                <div>
                                    <label for="reg_email" class="block text-sm font-semibold text-[#1E1E1E] mb-1.5">Email <span class="text-[#DD5D20]">*</span></label>
                                    <input type="email" id="reg_email" name="email" required
                                           class="w-full px-4 py-3 rounded-xl border border-[#FFAC93]/50 bg-white text-[#1E1E1E] focus:outline-none focus:ring-2 focus:ring-[#DD5D20] focus:border-[#DD5D20]/50">
                                </div>
                                <div>
                                ...
                                <button type="submit"
                                        class="w-full px-6 py-3.5 rounded-xl font-semibold text-white bg-[#DD5D20] hover:bg-[#D44A1F] focus:outline-none focus:ring-2 focus:ring-[#DD5D20] focus:ring-offset-2 transition-colors shadow-sm">
                                    Register
                                </button>
                ...
                <div class="mt-auto border-t border-[#FFA589]/20 py-8 bg-white/80">
                    <div class="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-10 text-center text-sm text-[#606060]">
                        <a href="{{ route('public.events') }}" class="text-[#DD5D20] font-medium hover:underline">← Back to all events</a>
                    </div>
                </div>
    ...
    @if(in_array($event->registration_type, ['simple', 'form'], true) && $event->isUpcoming())
        ...
        <script>
            (function () {
                ...
                const fieldCls = 'w-full px-4 py-3 rounded-xl border border-[#FFAC93]/60 bg-white text-[#1E1E1E] focus:outline-none focus:ring-2 focus:ring-[#DD5D20] focus:border-transparent';
                ...
                form.addEventListener('submit', async function (e) {
                    ...
                    if (!registerUrl) {
                        ...
                        msg.className = 'text-sm text-[#1E1E1E] bg-[#FFA589]/30 border border-[#DD5D20]/40 rounded-lg px-3 py-2';
                    ...
                    try {
                        ...
                        if (data.success) {
                            ...
                            msg.className = 'text-sm text-[#1E1E1E] bg-[#DD5D20]/25 border border-[#DD5D20]/40 rounded-lg px-3 py-2';
                            msg.classList.remove('hidden');
                            form.reset();
                        } else {
