                ...
                <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div class="flex justify-between h-20">
                        ...
                        <div class="flex items-center space-x-4">
                            ...
                            @if (Route::has('login'))
                                @auth
                                    <a href="{{ route('dashboard') }}" class="hidden md:block text-gray-200 hover:text-white px-3 py-2 rounded-md text-sm font-medium">
                                        Dashboard
                                    </a>
                                @else
                                    <a href="{{ route('login') }}" class="bg-[#DD5D20] hover:bg-[#C24E16] text-white px-4 py-2 rounded-md text-sm font-medium transition-colors">
                                        Login
                                    </a>
                                @endauth
                            @endif
                    ...
                    <div x-show="mobileMenuOpen" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95" class="md:hidden">
                        <div class="px-2 pt-2 pb-3 space-y-1 sm:px-3 bg-gray-800 rounded-lg mt-2">
                            ...
                            @if (Route::has('login'))
                                @auth
                                    <a href="{{ route('dashboard') }}" class="text-gray-200 hover:text-white block px-3 py-2 rounded-md text-base font-medium">
                                        Dashboard
                                    </a>
                                @endauth
                            @endif
